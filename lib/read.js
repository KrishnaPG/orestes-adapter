...
var Read = Juttle.proc.source.extend({
    ...
    start: function() {
        var self = this;
        var fetchers = [];
        return utils.startup().then(function get_all_fetchers() {
            var startMs = self.from.milliseconds();
            var endMs = self.to.milliseconds();
            var options = {series_limit: MAX_SERIES};

            function buffer_fetcher(fetcher) {
                fetchers.push(fetcher);
            }

            return Orestes.read(self.es_filter, self.space, startMs, endMs, options, buffer_fetcher);
        })
        .then(function() {
            ...
            function refill_streams() {
                ...
                if (streams.length === 0) {
                    ...
                    var streams_to_refill = streams.filter(needs_refill);

                    if (streams_to_refill.length === 0) {
                        throw new Error('uh oh, streams_to_refill should not be empty');
                    }

                    return Promise.map(streams_to_refill, function(stream) {
                        return stream.fetch();
            ...
            function loop() {
                ...
                // kick off fetch any I/O before we emit points so Cassandra can
                // get started while downstream Juttle processing runs
                var refill_promise = refill_streams();

                if (out.length > 0) {
                    self.emit(juttle_utils.toNative(out));
                }

                return refill_promise.then(function() {
                    if (!self.cancelled && streams.length > 0) {
                        return loop();
                    }
                });
            }
